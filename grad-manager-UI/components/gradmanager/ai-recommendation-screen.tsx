...
export function AiRecommendationScreen({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col bg-background">
      {/* Header */}
      <div className="shrink-0 space-y-3 px-4 pb-3 pt-4 border-b border-border bg-card">
              ...
              if (f.addOnly) {
                return (
                  <button
                    ...
                    className="flex items-center gap-1 rounded-full border border-dashed border-border px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent"
                  >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto bg-background">
        <div className="mx-auto w-full max-w-5xl px-4 pb-6 pt-3 space-y-4">
          <h2 className="text-sm font-semibold text-foreground">관심분야 기반 추천</h2>
...
function CourseCard({ course }: { course: AiCourse }) {
  ...
  return (
    <article
      className={`overflow-hidden rounded-2xl border-b-4 bg-card shadow-sm ${c.border}`}
    >
      <div className="flex items-center gap-3 p-4">
        {/* Credit badge */}
        <div
          className={`flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-full text-white ${c.badge}`}
        >
          <span className="text-base font-bold leading-none">{course.credit}</span>
          <span className="text-[10px] leading-tight">학점</span>
        </div>
      ...
      {/* Reason banner */}
      <div className={`flex items-center gap-2 px-4 py-2.5 ${c.banner}`}>
        <Star className={`h-4 w-4 shrink-0 ${c.bannerText}`} fill="currentColor" />
        <p className={`text-xs font-medium ${c.bannerText}`}>{course.reason}</p>
      </div>
    </article>
  )
}
